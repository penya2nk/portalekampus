	...
	*/
    ...
    public function changeView ($sender,$param) {                
        try {
            ...
            $str = "SELECT fp.no_formulir,fp.nama_mhs,fp.jk,fp.tempat_lahir,fp.tanggal_lahir,fp.kjur1,fp.kjur2,fp.ta,fp.idkelas,pm.theme FROM formulir_pendaftaran fp LEFT JOIN profiles_mahasiswa pm ON pm.no_formulir=fp.no_formulir WHERE fp.no_formulir='$nim'";
            ...
            $_SESSION['currentPageDetailFormulir']['DataMHS']=$this->DataMHS;
            ...
            if ($activeview == $this->MVDetailFormulir->ActiveViewIndex) {
                ...
                $this->redirect('spmb.DetailFormulir',true,array('id'=>$datamhs['nim']));
            }       
    ...
    public function itemCreatedRepeaterIPSdanIPK ($sender,$param) {
        ...
        if ($item->ItemType === 'Item' || $item->ItemType === 'AlternatingItem') {   
            CDetailFormulir::$totalM += $item->DataItem['jumlah_m'];
            CDetailFormulir::$totalSKS += $item->DataItem['jumlah_sks_ada_nilai'];
            $ipk=@ bcdiv(CDetailFormulir::$totalM,CDetailFormulir::$totalSKS,2);
            $item->literalIPK->Text=$ipk;
        }
    }   
    public function populateIPSdanIPK() {
        $nim=$_SESSION['currentPageDetailFormulir']['DataMHS']['nim'];        
    ...
    public function populateDulang() {
        $this->KRS->setDataMHS($_SESSION['currentPageDetailFormulir']['DataMHS']);
        $nim=$_SESSION['currentPageDetailFormulir']['DataMHS']['nim'];
	...
	public function itemCreatedRepeaterKRS ($sender,$param) {
        ...
        if ($item->ItemType === 'Item' || $item->ItemType === 'AlternatingItem') {   
            ...
            CDetailFormulir::$totalSKS=0;
            CDetailFormulir::$jumlahMatkul=0;
            while (list($k,$v)=each ($r)) {
                $v['kmatkul']=$this->Nilai->getKMatkul($v['kmatkul']);
                CDetailFormulir::$totalSKS+=$v['sks'];
                CDetailFormulir::$jumlahMatkul+=1;
                $result[$k]=$v;
            }           
    ...
    public function changeTheme ($sender,$param) {
        if ($this->IsValid) {
            ...
            $this->redirect('spmb.DetailFormulir',true,array('id'=>$nim));
        }
    }
    public function checkNIM ($sender,$param) {					
        ...
        if ($nim != '') {
            try {   
                if ($nim != $_SESSION['currentPageDetailFormulir']['DataMHS']['nim']) {
	...
	public function checkNIRM ($sender,$param) {
        ...
        if ($nirm != '') {
            try {   
                if ($nirm != $_SESSION['currentPageDetailFormulir']['DataMHS']['nirm']) {
    ...
    public function updateDataNIMNIRM ($sender,$param) {
		if ($this->Page->isValid) {	
            $nimasal=$_SESSION['currentPageDetailFormulir']['DataMHS']['nim'];
            ...
            $nama_mhs=$_SESSION['currentPageDetailFormulir']['DataMHS']['nama_mhs'];
