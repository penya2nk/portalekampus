...
class CTranskripAsli extends MainPageM {	
	public function onLoad ($param) {
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {			
            ...
            if (!isset($_SESSION['currentPageTranskripAsli'])||$_SESSION['currentPageTranskripAsli']['page_name']!='m.Akademik.TranskripAsli') {					
                $_SESSION['currentPageTranskripAsli']=array('page_name'=>'m.Akademik.TranskripAsli','page_num'=>0,'search'=>false,'tanggal_terbit'=>'none');												
            }
            $_SESSION['currentPageTranskripAsli']['search']=false;
	...
	public function Page_Changed ($sender,$param) {
		$_SESSION['currentPageTranskripAsli']['page_num']=$param->NewPageIndex;
		$this->populateData($_SESSION['currentPageTranskripAsli']['search']);
	}
    public function searchRecord ($sender,$param) {
		$_SESSION['currentPageTranskripAsli']['search']=true;
		$this->populateData($_SESSION['currentPageTranskripAsli']['search']);
	}
	public function populateData($search=false) {							
		...
		$this->RepeaterS->CurrentPageIndex=$_SESSION['currentPageTranskripAsli']['page_num'];		
		...
		if ($limit < 0) {$offset=0;$limit=10;$_SESSION['currentPageTranskripAsli']['page_num']=0;}
	...
	public function checkNim ($sender,$param){		
		...
		if ($nim != '') {
			try {				
				...
				if ($this->Application->getModule ('environment')->checkRequirementTranskripAsli) {
	...
	public function processKelulusan ($sender,$param) {
		if ($this->IsValid) {
			...
			$_SESSION['currentPageTranskripAsli']['m_TranskripAsli']=$this->Nilai->dataMhs;
			$this->redirect('nilai.TranskripAsli',true);
		}
	}
	public function detailProcess () {				
		$this->dataMhs=$_SESSION['currentPageTranskripAsli']['m_TranskripAsli'];		
		...
		if (isset($r[1])) {
			$bool=false;
			$_SESSION['currentPageTranskripAsli']['m_TranskripAsli']['dataTranskrip']=$r[1];
			...
			$this->systemErrorMessage = 'Proses tidak bisa dilaksanakan karena belum ada data Transkrip Asli.';
	...
	public function saveData ($sender,$param) {
		if ($this->IsValid) {
			...
			$log->getIdLogMaster();
			$nim=$_SESSION['currentPageTranskripAsli']['m_TranskripAsli']['nim'];
			$log->dataMhs['nim']=$_SESSION['currentPageTranskripAsli']['m_TranskripAsli']['nim'];
			$ta=$_SESSION['ta'];					
			...
			if ($no_transkrip == '') {                
				...
				if ($this->DB->insertRecord($str)) {					
					...
					foreach($this->RepeaterTranskripS->Items as $inputan) {						
						...
						if ($n_kual != '')
							$log->insertLogIntoTranskripAsli($kmatkul,$nmatkul,'input',$n_kual);
					}
					...
					foreach($this->RepeaterTranskripS->Items as $inputan) {						
						...
						if ($n_kual_sebelumnya != $n_kual) {				
							...
							if ($n_kual_sebelumnya == '')
								...
								$log->insertLogIntoTranskripAsli($kmatkul,$nmatkul,'update',$n_kual_sebelumnya,$n_kual);
						}
					}
					$this->DB->query('COMMIT');
				}else {
	...
	public function printTranskripperNim ($sender,$param) {
		if ($this->isValid) {
			...
			$this->Nilai->printTranskripAsli('pdf');				
	...
	public function closeTranskrip($sender,$param) {	
		unset($_SESSION['currentPageTranskripAsli']['m_TranskripAsli']);
		$this->redirect('nilai.TranskripAsli',true);
	}
}
