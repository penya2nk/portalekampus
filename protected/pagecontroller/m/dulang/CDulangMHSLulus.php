...
class CDulangMHSLulus Extends MainPageM {		
    ...
    public function populateData($search=false) {
        ...
        if ($search) {
            $str = "SELECT d.iddulang,vdm.no_formulir,vdm.nim,vdm.nirm,vdm.nama_mhs,vdm.iddosen_wali,d.tanggal,d.tahun,d.idsmt FROM v_datamhs vdm,dulang d WHERE vdm.nim=d.nim AND d.k_status='L' AND vdm.kjur='$kjur'";
            ...
            $str = "SELECT d.iddulang,vdm.no_formulir,vdm.nim,vdm.nirm,vdm.nama_mhs,vdm.iddosen_wali,d.tanggal,d.tahun,d.idsmt FROM v_datamhs vdm,dulang d WHERE vdm.nim=d.nim AND d.tahun=$ta AND d.idsmt=$idsmt AND vdm.kjur='$kjur' AND d.k_status='L' $str_dw $str_tahun_masuk";
        ...
        if ($nim != '') {
            try {
                if (!isset($_SESSION['currentPageDulangMHSLulus']['DataMHS']['no_formulir'])) {
                    ...
                    $datamhs=$r[1]; 
                    $datamhs['iddata_konversi']=$this->Finance->isMhsPindahan($nim,true);
