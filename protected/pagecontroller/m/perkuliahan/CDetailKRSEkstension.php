...
class CDetailKRSEkstension extends MainPageM {
	/**
	* total SKS
	*/
	static $totalSKS=0;
	...
	*/
	...
	public function onLoad($param) {
		...
		if (!$this->IsPostBack&&!$this->IsCallback) {	
            if (!isset($_SESSION['currentPageDetailKRSEkstension'])||$_SESSION['currentPageDetailKRSEkstension']['page_name']!='mh.perkuliahan.DetailKRSEkstension') {
				$_SESSION['currentPageDetailKRSEkstension']=array('page_name'=>'mh.perkuliahan.DetailKRSEkstension','page_num'=>0,'DataKRS'=>array(),'DataMHS'=>array());
			} 
    ...
    public function itemCreated ($sender,$param) {
        ...
        if ($item->ItemType === 'Item' || $item->ItemType === 'AlternatingItem') {                
            CDetailKRSEkstension::$totalSKS+=$item->DataItem['sks'];
            CDetailKRSEkstension::$jumlahMatkul+=1;
        }
    }
	protected function populateData () {
        try {			
            $idkrs=addslashes($this->request['id']);            				
            $str = "SELECT vdm.no_formulir,vdm.nim,vdm.nirm,vdm.nama_mhs,vdm.jk,vdm.tempat_lahir,vdm.tanggal_lahir,vdm.kjur,vdm.nama_ps,vdm.idkonsentrasi,k.nama_konsentrasi,vdm.tahun_masuk,iddosen_wali,vdm.idkelas,vdm.k_status,sm.n_status AS status,krs.idsmt,krs.tahun,krs.tasmt,krs.sah,vdm.idkelas,ke.nkelas FROM krs LEFT JOIN v_datamhs vdm ON (krs.nim=vdm.nim) LEFT JOIN konsentrasi k ON (vdm.idkonsentrasi=k.idkonsentrasi) LEFT JOIN status_mhs sm ON (vdm.k_status=sm.k_status) LEFT JOIN kelas ke ON (vdm.idkelas=ke.idkelas) WHERE krs.idkrs='$idkrs'";
            $this->DB->setFieldTable(array('no_formulir','nim','nirm','nama_mhs','jk','tempat_lahir','tanggal_lahir','kjur','nama_ps','idkonsentrasi','nama_konsentrasi','tahun_masuk','iddosen_wali','idkelas','k_status','status','idsmt','tahun','tasmt','sah','idkelas','nkelas'));
            $r=$this->DB->getRecord($str);	           
            $datamhs=$r[1];
            if (!isset($r[1])) {
                $_SESSION['currentPageDetailKRSEkstension']['dataKRS']=array();
            ...
            $_SESSION['currentPageDetailKRSEkstension']['DataMHS']=$datamhs;
            $this->KRS->setDataMHS($datamhs);
            $this->KRS->getKRS($_SESSION['ta'],$_SESSION['semester']);                                                                        
            $_SESSION['currentPageDetailKRSEkstension']['DataKRS']=$this->KRS->DataKRS;
	...
	public function printKRS ($sender,$param) {
        ...
        switch ($_SESSION['outputreport']) {
            ...
            case  'pdf' :                
                ...
                $dataReport=$_SESSION['currentPageDetailKRSEkstension']['DataMHS'];
                $dataReport['krs']=$_SESSION['currentPageDetailKRSEkstension']['DataKRS']['krs'];        
                $dataReport['matakuliah']=$_SESSION['currentPageDetailKRSEkstension']['DataKRS']['matakuliah'];        
